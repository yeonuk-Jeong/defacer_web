<template>
  <div id="dwvResult">
    <div class="layerContainer">
      <h1>Result Dicom File</h1>
      <div class="dropBox dropBoxBorder md-body-1">
          <p>This area will show the result data</p>
      </div>
    ...
    <div style="text-align:center;">
      <v-btn style="background-color:#01DFA5;color:whitesmoke;margin-top:30px;" @click="downloadResult">Download File</v-btn>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    downloadResult: function() {
      if(this.url === null){
        alert("Please, follow the instruction above. there is no result file yet.");
        return;
      }
      this.$http({
        method: 'get',
    ...
    next: function() {
      if(this.index+1 < this.files.length-1){
        ...
        this.dwvApp.loadURLs([this.file]);
      }else{
        console.log("end");
      }
    },
